...
gh pr create -R fiazsami/helios --base master --draft   # a draft, always
```

Pass `-R fiazsami/helios` explicitly. This repo is a fork, and `gh` resolves the
base repository to the **parent** in a fork clone — without it you can open a
pull request on the upstream project by accident.

**Open every PR as a draft, and stop at `gh pr ready -R fiazsami/helios`.** An open PR says nothing
...
`gh pr ready -R fiazsami/helios` only once review is clean; merging is a separate decision taken
...
This repository shares a C++ core with `hyperspace` — they are twins, and `hills` is not one. A change to that core is a PR
...
superproject — `tools/coverage.sh` and `coverage/helios.json`, which hold the source
