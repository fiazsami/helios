...
TEST(matrix_rotationInvert_does_not_actually_invert_the_rotation)
{
     ...
     * correct transpose. This is a real discrepancy, distinct from the two
     * bugs called out in the task (ss-qss's slerp and the rgb2hsl pair); no
     * bd issue is filed for it yet. Pinned as found, not fixed. */
    rsQuat q;
...
{
    /* Characterization of a real bug (found while writing this test, not one
     * of the two named in the task; no bd issue filed yet). For trace <= 0,
     * fromMat picks the dominant diagonal axis i and is supposed to write
     * q[i] = sqrt(...) * 0.5. Instead it writes `q[i] *= 0.5f` -- it reads
     * whatever q[i] already held *before* this call and halves that, rather
     * than assigning the freshly computed value. On a default-constructed
     * rsQuat (q[i] == 0 for i in 0..2), that means the dominant component
     * always comes out 0, no matter what the input matrix says it should be.
     * All three axis branches (i==0, i==1, i==2) show the same pattern. */
...
{
    /* Sharper version of the same bug: seed the target quaternion with an
     * arbitrary prior x component (9.0) instead of the default 0, and the
     * dominant-x branch's output x is exactly half of it (4.5) -- the input
     * matrix has no say in that component at all. */
    rsQuat correct;
...
{
    /* A second, independent defect in the same branch, visible once the axis
     * is not conveniently aligned so the "wrong" component isn't coincidentally
     * zero anyway: the formula for the two non-dominant, non-w components
     * pulls the wrong pair of matrix entries, so they come out neither
     * matching the source rotation nor the earlier all-zero pattern. Pinned
     * as observed. */
    rsQuat correct;
